  ...
  if (message.content.match(mentionRegex)) {
    message.channel.send(
      ...
      .setDescription(`Hey <@${message.author.id}>, My prefix for this guild is \`\`\`${Prefix}\`\`\`.Use \`\`\`${Prefix}help\`\`\` or <@${bot.user.id}> help to get a list of commands`)
       .setColor("RANDOM")
       .setFooter(`Вызвал: ${message.author.username}`)
       .setTimestamp()
  )};
...
function decodeMs(num) {
  ...
  bot.on("Бот Запустился!", message => {
    bot.channels.cache.get('947915662968389632').send(message.content);
})

bot.login("OTQwMDE4NzY3MjMxMDU4MDEw.YgBSsg.UMNyj1Gd0Lx50VKGH1OUC52ThIY"); // Авторизация бота